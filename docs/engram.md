...
A relational row is: schema, fields, values. It has no concept of importance. No concept of time-weighted relevance. No native associations to other rows. No decay.
...
But graph edges are static. You define them, they stay the same until you change them. They do not strengthen when you traverse them repeatedly. They do not weaken when they go dormant. Nodes do not decay in relevance. The graph does not push to you when something changes.
...
The current Ebbinghaus retention score. This is what the decay worker continuously recomputes. It represents how present this memory is right now, based on how recently and how often it has been accessed relative to its stability. Relevance is not permanent. It must be earned through use.

**Stability (float32, in days)**
How resistant this engram is to decay. Higher stability means slower decay. Stability increases with access frequency and — critically — access *pattern*. An engram accessed 50 times over six months is more stable than one accessed 50 times in a single day. The spacing effect is built in.
...
The cognitive workers — decay, Hebbian, contradiction, confidence — need to read and update cognitive scores (relevance, confidence, stability, access count, timestamps) continuously. These updates are small and frequent. If they required deserializing the entire record, including the content field which can be up to 16KB and compressed, the overhead would be substantial and constant.
...
**Metadata (100 bytes, fixed)**
The ID (16 bytes), all timestamps (24 bytes), all cognitive scores — confidence, relevance, stability — as `float32` values (12 bytes), access and association counts (5 bytes), lifecycle state (1 byte), and 32 bytes reserved for future use. This entire block is at a known, stable offset. A cognitive worker updating relevance after a decay computation touches exactly these bytes and nothing else.
